...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="flex items-center justify-between mb-6">
        ...
        @if($canEdit)
        <a href="{{ route('material-costs.edit', $log) }}" class="btn-primary">
            ✎ Edit Record
        </a>
        @endif
    </div>
    ...
    <div class="card mb-6">
        ...
        <div class="grid grid-cols-3 gap-6">
            <div class="bg-gray-50 rounded-lg p-6 border border-gray-200">
                ...
                <p class="text-3xl font-bold text-blue-600 mt-2">{{ number_format($log->used_qty, 2) }}</p>
            </div>

            <div class="bg-gray-50 rounded-lg p-6 border border-gray-200">
                <p class="text-sm font-medium text-gray-600">Cost per Item</p>
                <p class="text-3xl font-bold text-indigo-600 mt-2">${{ number_format($log->cost_per_item, 2) }}</p>
            </div>

            <div class="bg-red-50 rounded-lg p-6 border border-red-200">
                <p class="text-sm font-medium text-gray-600">Total Cost</p>
                <p class="text-3xl font-bold text-red-600 mt-2">${{ number_format($log->total, 2) }}</p>
            </div>
        </div>
        ...
        <div class="mt-6 bg-blue-50 border border-blue-200 rounded-lg p-6">
            ...
            <div class="flex items-center justify-center gap-4 text-lg">
                ...
                <span class="text-indigo-600 font-semibold">${{ number_format($log->cost_per_item, 2) }}/item</span>
                <span class="text-2xl text-gray-400">=</span>
                <span class="text-red-600 font-bold text-2xl">${{ number_format($log->total, 2) }}</span>
            </div>
        </div>
    </div>
    ...
    <div class="flex items-center gap-4 mb-6">
        ...
        @endif
        <a href="{{ route('material-costs.index', ['project_id' => $project->id]) }}" class="btn-secondary">
            ← Back to Logs
        </a>
